...
for idx, filepath in enumerate(model_files, 1):
    print(f"[{idx}/{len(model_files)}] Loading: {filepath.name}")

    try:
        model = onnx.load(str(filepath))
        onnx.checker.check_model(model)

        models_info[filepath.name] = {"path": filepath, "model": model, "valid": True}

        print(f"  Status: Valid\n")

    except onnx.checker.ValidationError as e:
        print(f"  Validation failed: {str(e)}\n")
        models_info[filepath.name] = {"valid": False, "error": str(e)}
